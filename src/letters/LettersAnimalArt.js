...
function headwear(id){
  ...
  const cap=`<g class="fitted-cap"><path class="prod-acc" d="M${h.left} ${h.base-8}C${h.left+8} ${h.top+14} ${mid-35} ${h.top} ${mid} ${h.top}C${mid+35} ${h.top} ${h.right-8} ${h.top+14} ${h.right} ${h.base-8}L${h.right-8} ${h.base+4}Q${mid} ${h.base-9} ${h.left+8} ${h.base+4}Z" fill="#e69b77"/><path d="M${mid-7} ${h.top+9}Q${mid-29} ${h.top+19} ${mid-31} ${h.base-17}" fill="none" stroke="#f6c7a2" stroke-width="8" stroke-linecap="round"/><path d="M${mid+4} ${h.top+9}Q${mid+22} ${h.top+22} ${mid+25} ${h.base-15}" fill="none" stroke="#bf795c" stroke-width="4" stroke-linecap="round"/><circle cx="${mid}" cy="${h.top+3}" r="7" fill="#efd391" stroke="#9c7650" stroke-width="3"/><path class="prod-acc cap-visor" d="M${h.left-14} ${h.base-8}Q${mid} ${h.base-27} ${h.right+12} ${h.base-8}Q${h.right+37} ${h.base+1} ${h.right+10} ${h.base+13}Q${mid} ${h.base+1} ${h.left-15} ${h.base+13}Q${h.left-34} ${h.base+4} ${h.left-14} ${h.base-8}Z" fill="#ce855f"/><path d="M${h.left-10} ${h.base-3}Q${mid} ${h.base-20} ${h.right+9} ${h.base-3}" fill="none" stroke="#f1b68a" stroke-width="4" stroke-linecap="round"/></g>`;
  const taq=`<path class="prod-acc" d="M${t.left} ${t.base}C${t.left+6} ${t.top+12} ${mid-31} ${t.top} ${mid} ${t.top}C${mid+31} ${t.top} ${t.right-6} ${t.top+12} ${t.right} ${t.base}L${t.right-3} ${t.base+10}Q${mid} ${t.base-2} ${t.left+3} ${t.base+10}Z" fill="#fff6de"/><path d="M${t.left+8} ${t.base-3}Q${mid} ${t.base-14} ${t.right-8} ${t.base-3}" fill="none" stroke="#b1b995" stroke-width="11"/><path d="M${t.left+12} ${t.base-3}Q${mid} ${t.base-14} ${t.right-12} ${t.base-3}" fill="none" stroke="#fff4cd" stroke-width="3" stroke-dasharray="3 8"/><path d="M${mid-7} ${t.top+9}Q${mid-27} ${t.top+18} ${t.left+20} ${t.base-18}M${mid+7} ${t.top+9}Q${mid+27} ${t.top+18} ${t.right-20} ${t.base-18}" fill="none" stroke="#dacda7" stroke-width="3" stroke-linecap="round"/>`;
  const crown=`<path class="prod-acc" d="M${c.left} ${c.base}V${c.base-42}L${c.left+(c.right-c.left)*.25} ${c.base-20}L${mid} ${c.peak}L${c.right-(c.right-c.left)*.25} ${c.base-20}L${c.right} ${c.base-42}V${c.base}Z" fill="#edca6a"/><path d="M${c.left+5} ${c.base-12}H${c.right-5}V${c.base-3}H${c.left+5}Z" fill="#cba052"/><path d="M${c.left+9} ${c.base-18}H${c.right-9}" stroke="#fff0b1" stroke-width="4" stroke-linecap="round"/><path d="M210 ${c.base-31}l8 9-8 9-8-9Z" fill="#d99591" stroke="#b58469" stroke-width="3"/><path d="M207 ${c.base-24}l3-4" stroke="#f9d6bd" stroke-width="2" stroke-linecap="round"/>`;
  const root=f.crownY,stemTop=Math.max(24,root-30);
  const sprout=`<ellipse class="prod-acc" cx="210" cy="${root}" rx="17" ry="8" fill="#af9164"/><path d="M199 ${root-2}Q210 ${root-7} 221 ${root-2}" stroke="#dcc28d" stroke-width="3" fill="none"/><path d="M210 ${root-3}Q207 ${root-18} 211 ${stemTop}" stroke="#719252" stroke-width="7" fill="none" stroke-linecap="round"/><path class="prod-acc" d="M211 ${stemTop+2}q-27-2-31-22 24 0 31 22Z" fill="#8ead6d"/><path class="prod-acc" d="M211 ${stemTop+2}q28-5 32-22-25 2-32 22Z" fill="#b8ce87"/><path d="M190 ${stemTop-10}L209 ${stemTop}M214 ${stemTop}L233 ${stemTop-11}" fill="none" stroke="#d9e5ae" stroke-width="3" stroke-linecap="round"/>`;
...
function foregroundAccessories(id){
  ...
  const bow=item('bow',`<g transform="translate(${sideX} ${sideY}) rotate(-18)"><path class="prod-acc" d="M-5 0Q-26-28-38-24Q-44 0-38 24Q-22 22-5 0M5 0Q26-28 38-24Q44 0 38 24Q22 22 5 0" fill="#df9dad"/><path d="M-29-11L-13-3M-28 12L-12 4M29-11L13-3M28 12L12 4" stroke="#b9738d" stroke-width="4" stroke-linecap="round"/><rect class="prod-acc" x="-9" y="-12" width="18" height="24" rx="7" fill="#bd7e94"/><path d="M-3-7V7" stroke="#ecc1c7" stroke-width="3" stroke-linecap="round"/></g>`);
  const petals=[0,72,144,216,288].map(a=>`<ellipse rx="13" ry="22" transform="rotate(${a} ${sideX} ${sideY}) translate(${sideX} ${sideY-19})" fill="#e4a1b1" stroke="#ad7687" stroke-width="3"/>`).join('');
  const flower=item('flower',`${petals}<circle class="prod-acc" cx="${sideX}" cy="${sideY}" r="13" fill="#efd080"/><circle cx="${sideX-4}" cy="${sideY-5}" r="4" fill="#fff0bd"/>`);
  const glasses=item('glasses',`<g fill="none" stroke="#80694d" stroke-width="7" stroke-linecap="round"><rect x="${e.left-e.rx}" y="${180-e.ry}" width="${e.rx*2}" height="${e.ry*2}" rx="${Math.round(e.rx*.68)}"/><rect x="${e.right-e.rx}" y="${180-e.ry}" width="${e.rx*2}" height="${e.ry*2}" rx="${Math.round(e.rx*.68)}"/><path d="M${e.left+e.rx} 177Q210 169 ${e.right-e.rx} 177M${e.left-e.rx} 168l-18-7m${e.right+e.rx} 168 18-7"/></g><g fill="none" stroke="#d6bd8d" stroke-width="3" stroke-linecap="round">${[e.left,e.right].map(x=>`<path d="M${x-e.rx+13} ${180-e.ry+9}Q${x-18} ${180-e.ry-1} ${x+5} ${180-e.ry+1}"/>`).join('')}</g>`);
