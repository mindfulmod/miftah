(function (ns) {
  class WorldMap {
    ...
    // A biome island is "open" once any of its zones has hatched an animal
    // (zoneState becomes "unlocked"). Until then it is "locked" and its bridge
    // gate stays in place. Returns "open" for unknown islands so nothing breaks.
    islandState(islandId, progress) {
      const island = this.islandById.get(islandId);
      if (!island || !progress) return "open";
      const anyOpen = island.zones.some((zone) => progress.zoneState(zone) === "unlocked");
      return anyOpen ? "open" : "locked";
    }
    ...
    isPropActive(prop, progress) {
      ...
      if (prop.lockIsland) return this.islandState(prop.lockIsland, progress) === "locked";
